...
dbModule.createNewDevice = function (deviceObj, callback) {
	...
	if (!deviceObj.gateway_tag) {
		objectToStore.gateway_tag = 'NO GATEWAY TAG';
	}
	...
	if (!deviceObj.gateway_address) {
		objectToStore.gateway_address = 'NO GATEWAY ADDRESS';
	}
...
dbModule.updateDeviceByID = function (id, deviceObj, callback) {
	...
	//Check if deviceObj.device_tag exists
	if (deviceObj.device_tag) {
		query.push('device_tag=:device_tag');
		valueObject.device_tag = deviceObj.device_tag;
	}
	...
	//Check if deviceObj.device_address exists
	if (deviceObj.device_address) {
		query.push('device_address=:device_address');
		valueObject.device_address = deviceObj.device_address;
	}
